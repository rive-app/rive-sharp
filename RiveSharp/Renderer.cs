...
namespace RiveSharp
{
    ...
    public class Renderer
    {
        ...
        private SKCanvas mSKCanvas;

        public Renderer(SKCanvas mSKCanvas)
        {
            this.mSKCanvas = mSKCanvas;
        }

        public void Save() { mSKCanvas.Save(); }
        public void Restore() { mSKCanvas.Restore(); }

        public void Transform(Mat2D m)
        {
            var mat = new SKMatrix(m.x1, m.x2, m.tx, m.y1, m.y2, m.ty, 0, 0, 1);
            mSKCanvas.Concat(ref mat);
        }

        public void DrawPath(RenderPath path, RenderPaint paint)
        {
            mSKCanvas.DrawPath(path.SKPath, paint.SKPaint);
        }

        public void ClipPath(RenderPath path)
        {
            mSKCanvas.ClipPath(path.SKPath, SKClipOperation.Intersect, true);
        }

        public void DrawImage(RenderImage image, BlendMode blendMode, float opacity)
        {
            mSKCanvas.DrawImage(image.SKImage, 0, 0, new SKPaint
            {
                IsAntialias = true,
        ...
        public void DrawImageMesh(RenderImage image,
            ...
            mSKCanvas.DrawVertices(skVertices, SKBlendMode.Dst, new SKPaint
            {
